#!/usr/bin/env pwsh
#
# PowerShell script for deploying fixed diagnostic agent on Windows
#

Write-Host "Deploying Fixed Diagnostic Agent" -ForegroundColor Green
...
# Transfer to Pi
Write-Host "Transferring to Pi..." -ForegroundColor Yellow
scp -P 2222 $tarFile castlebravo@picklegate.ddns.net:~/

if ($LASTEXITCODE -eq 0) {
    Write-Host "✅ Transfer successful!" -ForegroundColor Green
    Write-Host ""
    Write-Host "Next steps on Pi:" -ForegroundColor Cyan
    Write-Host "1. SSH to Pi: ssh -p 2222 castlebravo@picklegate.ddns.net" -ForegroundColor White
    ...
    Write-Host "Check connection to picklegate.ddns.net:2222" -ForegroundColor Yellow
}
